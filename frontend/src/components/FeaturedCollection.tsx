        ...
        <section className="py-20 bg-white border-b border-gray-100">
            <div className="container mx-auto px-4">
                ...
                <div className="relative group">
                    ...
                    <button
                        onClick={() => scroll('left')}
                        className="hidden md:flex absolute left-0 top-[40%] -translate-y-1/2 -translate-x-3 md:-translate-x-5 z-20 w-12 h-12 bg-gray-100/90 rounded-full items-center justify-center text-gray-600 hover:text-gray-900 hover:bg-gray-200 transition-all duration-300 shadow-md opacity-0 group-hover:opacity-100 disabled:opacity-0"
                    ...
                    >
                        {FEATURED_COLLECTION.map((product) => (
                            <div
                                key={product.id}
                                className="flex-none w-[260px] md:w-[280px] lg:w-[300px] snap-start group/card relative"
                            >
                                <Link href="/products/detail" className="block">
                                    {/* Image Area */}
                                    <div className="relative aspect-[3/4] overflow-hidden rounded-sm mb-4 md:mb-5 bg-gray-100">
                                        <img
                                            src={product.image}
                                            alt={product.title}
